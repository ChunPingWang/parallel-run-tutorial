...
> ⚠ **不要假設你的 CNI 會執行 NetworkPolicy。** CNI 若沒實作，所有政策會「套上去
> 卻完全不擋」，而 `kubectl get networkpolicy` 仍顯示規則存在——那是最危險的假通過。
>
> 較早版本的 kindnet（kind 預設 CNI）確實不執行 NetworkPolicy。**但 kind v0.33
> 的 kindnetd（`v20260528`）已經會執行**——同一份 policy 在它上面實測也擋得住。
> 版本之間會變，所以下面固定用 Calico 建立叢集以取得可重現的結果；若你用其他
> 環境，請以本工具的執行結果為準，不要以 CNI 名稱推論。
>
> 判讀方式：若「阻斷」項**全數**失敗（實測 `reachable=yes`），第一個要懷疑的就是
> CNI 沒有執行 NetworkPolicy。
